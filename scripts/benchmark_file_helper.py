...
def get_model_path(bench_params: BenchmarkParams):
    return '{0}/nn_weights-{1}-{2}-{3}.hdf5'.format(bench_params.save_model_path, bench_params.id,
...
def copy_best_and_cleanup_files(bench_params: BenchmarkParams, max_index, rounded_acc):
    ...
    if bench_params.save_files:
        copyfile(get_model_path(bench_params),
                 '{0}/nn_weights-{1}-{2}-accuracy-{3}.hdf5'.format(model_path, id, sym, rounded_acc))
        copyfile('{0}/nn-{1}-{2}-{3}.png'.format(bench_params.save_img_path, id, sym, max_index + 1),
                 '{0}/nn-{1}-{2}-accuracy-{3}.png'.format(bench_params.save_img_path, id, sym, rounded_acc))
        copyfile('{0}/nn-{1}-{2}-{3}.pdf'.format(bench_params.save_img_path, id, sym, max_index + 1),
                 '{0}/nn-{1}-{2}-accuracy-{3}.pdf'.format(bench_params.save_img_path, id, sym, rounded_acc))

        if bench_params.cleanup_files:
            for f in os.listdir(model_path):
                if re.search('nn_weights-{0}-{1}-\d+\.hdf5'.format(id, sym), f):
                    os.remove(os.path.join(model_path, f))

            for f in os.listdir(bench_params.save_img_path):
                if re.search('nn-{0}-{1}-\d+\.(png|pdf)'.format(id, sym), f):
