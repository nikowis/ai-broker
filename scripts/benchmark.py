...
class Benchmark:
    ...
    def run(self, x_train, x_test, y_train, y_test, bench_params, results_df, sym):
        ...
        while curr_iter_num < bench_params.iterations:
            ...
            model, history, accuracy, loss, y_test_prediction = self.learn(model, bench_params, callbacks, model_path,
                                                                           x_train,
                                                                           x_test,
                                                                           y_train, y_test)
        ...
        if bench_params.save_files:
            ...
            copyfile(best_model_of_all_path,
                     '{0}/nn_weights-{1}-{2}-accuracy-{3}.hdf5'.format(bench_params.save_model_path, bench_params.id,
                                                                       sym,
    ...
    def learn(self, model, bench_params: benchmark_params.NnBenchmarkParams, callbacks, model_path, x_train, x_test,
              y_train, y_test):
        if bench_params.walk_forward_testing:
...
if __name__ == '__main__':
    ...
    bench = Benchmark(['GOOGL', 'AMZN'], bench_params, {'epochs': [20],
                                                        'layers': [[]]})
