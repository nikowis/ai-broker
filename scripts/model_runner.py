...
def run(x_train, x_test, y_train, y_test, bench_params):
    ...
    for iter in range(learning_params.iterations):
        curr_iter_num = iter+1
        iter_time = time.time()

        model = nn_model.create_seq_model(x_train.shape[1], bench_params.model_params)
        
        mcp_save = ModelCheckpoint(SAVE_MODEL_PATH + 'nn_weights-' + learning_params.id + '-' + str(curr_iter_num) + '.hdf5', save_best_only=True,
