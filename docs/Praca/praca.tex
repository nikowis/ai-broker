...
Giełda jest miejscem przyciągającym rzesze inwestorów pragnących pomnożyć swój kapitał. Właściwy handel instrumentami finansowymi na giełdzie pozwala na osiąganie dochodów przekraczających zyski z~alternatywnych inwestycji takich jak na przykład nieruchomości. Skuteczna inwestycja na giełdzie zazwyczaj opiera się o wybór odpowiednich spółek. Jednak przy aktywnym zarządzaniu portfelem akcji podejmowane decyzje o sprzedaży i kupnie są ważniejsze od doboru spółek. Wiąże się to oczywiście z dużym ryzykiem i wymaga wiedzy z~dziedziny ekonomii. Co jeżeli udałoby się opracować model decyzyjny, umożliwiający grę na giełdzie  bez wiedzy eksperckiej? 

\bigskip

Celem niniejszej pracy było stworzenie algorytmu decyzyjnego, który umożliwi skuteczne zarządzanie portfelem inwestycyjnym dowolnej osobie. W pracy opisano charakterystykę giełdy, dostępne dane giełdowe oraz różne podejścia do tematu przewidywania cen na giełdzie. Podjęto ponadto próbę stworzenia algorytmów do prognozowania za pomocą modeli z~dziedziny uczenia maszynowego. W pracy zostały wykorzystane takie modele, jak: sztuczne sieci neuronowe, maszyny wektorów nośnych, lasy losowe, czy też metoda \textit{Light GBM}. Do oceny i~porównywania jakości modeli użyto miary dokładności oraz miary \textit{AUC}. Następnie, w celu dalszej oceny skuteczności, wykonano symulację zautomatyzowanego handlu na giełdzie za pomocą prostego algorytmu decyzyjnego wykorzystującego stworzone modele. 
...
\begin{flushright}
Warszawa, dnia \hdashrule{3cm}{1pt}{1pt}
\end{flushright}
\bigskip
...
Wielu naukowców pracowało nad predykcją dokładnej wartość ceny akcji na przykład za pomocą sieci radialnych (Guo \& Wang \& Yang \& Miller, 2015 \cite{paper_regression_radial}). Często można spotkać prace traktujące o problemie wyznaczania trendu ceny, a więc problemu klasyfikacji. Klasyfikacja trendu giełdowego za pomocą maszyn wektorów nośnych została opisana między innymi przez \textit{Kim} (2003) \cite{paper_classification_svm}. Większość prac skupia się jednak na przewidywaniu trendu indeksów, aniżeli konkretnych spółek (Kara \& Boyacioglu \& Baykan, 2011 \cite{paper_classification_index_istanbul}; Patel \& Shah \& Thakkar \& Kotecha, 2015 \cite{paper_classification_index_sp500}). Według wiedzy autora tej pracy, brak jest źródeł naukowych opisujących prognozy dla konkretnych spółek giełdowych, wraz z~wykorzystaniem przewidywanych wartości do wspomagania decyzji giełdowych. 
...
Najsłynniejszym analitykiem fundamentalnym jest \textit{Warren Buffet}, prezes amerykańskiego holdingu \textit{Berkshire Hathaway \cite{berkeshire}}. 
...
Istnieje szereg algorytmów mogących posłużyć do zagadnień klasyfikacji lub regresji. W~tym rozdziale omówione zostały klasyfikatory, które później zostały użyte do predykcji na danych giełdowych.
...
\begin{figure}[H]%
\centering
\subfigure[Konstrukcja drzewa algorytmu \textit{XGBoost}.]{%
\label{img:level-wise}
\includegraphics[scale=0.7]{img/lgbm-level-wise.png}}%
\qquad
\subfigure[Konstrukcja drzewa algorytmu \textit{Light GBM}.]{%
\label{img:leaf-wise}
\includegraphics[scale=0.7]{img/lgbm-leaf-wise.png}}%
...
\begin{lstlisting}[caption={Funkcja \textit{manage\_account()} zarządzająca portfelem w symulacji giełdowej (plik \textit{market\_simulation.py}).}, label={code:simulation_manage_account},frame=single, captionpos=b, mathescape=true]
def manage_account(self, day, y_predicted_value, test_df):
    transaction_performed = False	
    ...
    # initialize next_day_open_price
    ...
...
Przykładowe wywołania symulacji dla każdego z modelów zostały zaprezentowane w listingach \ref{code:svmsim}, \ref{code:mlpsim}, \ref{code:rfsim} oraz \ref{code:lgbmsim}.

\begin{lstlisting}[caption={Przykładowe wywołanie symulacji giełdy dla modelu \textit{SVM} (plik \textit{market\_simulation.py}).}, label={code:svmsim},frame=single, captionpos=b, mathescape=true]
 bench_params = SVMBenchmarkParams(True, benchmark_name='svm_sim')
